...
class KVCache():
    ...
    def num_items(self) -> int:
        if len(self.key_cache) == 0:
            ...
            return self.key_cache[0].shape[-2] # Seq_len
...
def apply_rotary_emb(q, k, cos, sin, unsqueezed_dim=1):
    cos = cos.unsqueeze(unsqueezed_dim)
    sin = sin.unsqueeze(unsqueezed_dim)
...
class GemmaAttention(nn.Module):
    ...
    def forward(self,
        ...
        # shape: [Batch_size, Seq_len_Q, Num_heads_Q, Head_Dim] ->[Batch_size, Seq_len_Q, Num_heads_Q * Head_Dim]
...
class GemmaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: GemmaConfig, layer_ids: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = GemmaAttention(config=config, layer_idx=layer_ids)
